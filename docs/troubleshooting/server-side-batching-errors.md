...
When `max_batch_size` and `batch_interval` fields are set for the [Realtime API TensorFlow Predictor](), errors can be encountered if the associated model hasn't been built for batching.
